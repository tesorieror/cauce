 ...
 * @generated
 */
public class PointToPointInformationFlowItemSemanticEditPolicy extends
		InformationFlowBaseItemSemanticEditPolicy {

	/**
	 * @generated
	 */
	public PointToPointInformationFlowItemSemanticEditPolicy() {
	...
	protected Command getDestroyElementCommand(DestroyElementRequest req) {
		...
		for (Iterator<?> it = view.getTargetEdges().iterator(); it.hasNext();) {
			...
			if (InformationFlowVisualIDRegistry.getVisualID(incomingLink) == EntityToPointToPointInformationFlowEdgeEditPart.VISUAL_ID) {
		...
		for (Iterator<?> it = view.getSourceEdges().iterator(); it.hasNext();) {
			...
			if (InformationFlowVisualIDRegistry.getVisualID(outgoingLink) == PointToPointInformationFlowToEntityEdgeEditPart.VISUAL_ID) {
				DestroyElementRequest r = new DestroyElementRequest(
	...
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (InformationFlowVisualIDRegistry.getVisualID(node)) {
			case PointToPointInformationFlowP2POutputDataEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit
						.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (InformationFlowVisualIDRegistry.getVisualID(cnode)) {
